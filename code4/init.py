...
def homepage():
	...
	return render_template('Home.html', Departure=list_Departure_Airport, Arrival=list_Arrival_Airport, Airline_N= list_Airlines, Flight_num=list_flight_number,logged_in=logged_in, staff=staff, nonlogged=nonLogged)
...
def OneWay():
	...
	set_price= 'UPDATE Flight NATURAL JOIN price_adjust SET BasePrice= BasePrice*1.25 WHERE Seats_Ratio > 0.8 '
	cursor.execute(set_price)
	conn.commit()

	get_flight_data = 'SELECT * FROM Airplane NATURAL JOIN Flight JOIN availability_flight USING(Seats) WHERE Departure_Airport=%s AND Departure_date=%s AND Arrival_Airport=%s AND  Flight_status !="Canceled" And Avalible_seats > 0'
	...
	return render_template('Flight.html', Flight=data, dir=dir)
...
def Roundtrip():
	...
	get_flight_data = 'SELECT * FROM Airplane NATURAL JOIN Flight WHERE Departure_Airport=%s AND Departure_date=%s AND Arrival_Airport=%s AND  Flight_status !="Canceled"'
	cursor.execute(get_flight_data, (Departure_Airport,Departure_date, Arrival_Airport))
	data = cursor.fetchall()
...
def purchaseAuth():
	...
	for i in range(num_tickets):
		...
		Purchase_date=str(cursor.fetchone())
		

		get_time='SELECT CURTIME()'
		cursor.execute(get_time)
		Purchase_time=str(cursor.fetchone())
	...
	return render_template('CustomerAccount.html', num_tickets=num_tickets,total_amount=total_amount)
...
def get_flight_info():
	...
	get_past_flight= "SELECT Ticket_ID, Airline_Name, Flight_Number,Departure_Airport,Departure_date,Arrival_Airport, Arrival_date, Passenger_Fname, Passenger_Lname, final_price, Flight_status FROM Purchase NATURAL JOIN Ticket NATURAL JOIN Flight WHERE Customer_Email = %s and Departure_date < CURRENT_DATE"
